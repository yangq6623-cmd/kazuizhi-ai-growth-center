"""HTTP bridge for the guarded R8-15 SEO public deployment connector."""
...
from integrations.seo_public_deployer import configure, deploy_pending, status
...
def install():
    ...
    def do_get(handler):
        ...
        if path == "/api/r8-15/seo-public-deploy/status":
            try:
    ...
    def do_post(handler):
        path = urlsplit(handler.path).path
        if path not in {
            "/api/r8-15/seo-public-deploy/config",
            "/api/r8-15/seo-public-deploy/run",
        }:
        ...
        try:
            ...
            if path.endswith("/config"):
                ...
                handler._json_ok(deploy_pending(limit=max(1, min(20, int(payload.get("limit") or 10)))))
        except (OSError, ValueError, RuntimeError, TypeError, KeyError, json.JSONDecodeError) as error:
            handler._json_error(400, error)

    server.DashboardHandler.do_GET = do_get
    server.DashboardHandler.do_POST = do_post
    server.DashboardHandler._kz_r8_15_seo_public_deploy = True
    _INSTALLED = True
