...
def _read_registry():
    ...
    if not isinstance(value, dict) or value.get("schema") not in LEGACY_SCHEMAS:
...
def _read_devices():
    ...
    if not isinstance(value, dict) or value.get("schema") not in LEGACY_DEVICE_SCHEMAS:
...
def _append_audit(kind, *, account_id=None, device_id=None, detail=None):
    ...
    payload["events"].insert(0, {
        "at": now_iso(), "kind": kind, "account_id": account_id,
        "device_id": device_id, "detail": detail if isinstance(detail, dict) else {},
    })
...
def _save_checkpoint(registry, device_pool, reason):
    ...
    if quality[0] == 0 and quality[3] == 0:
        return
    ...
    # Authorization may legitimately expire; identity/device cardinality may not
    # silently shrink because of a transient/upgrade state.
    if pquality[0] > quality[0] or pquality[3] > quality[3]:
        return
    write_json(CHECKPOINT_PATH, {
        "schema": "kz.account-assets-checkpoint.v1",
        "saved_at": now_iso(), "reason": str(reason or "state_change")[:120],
...
def _merge_missing(current, fallback, key):
    ...
    id_key = "account_id" if key == "accounts" else "device_id"
    by_id = {str(x.get(id_key) or ""): x for x in rows if isinstance(x, dict)}
    restored = 0
    for candidate in fallback.get(key, []) if isinstance(fallback, dict) else []:
        ...
        if existing is None:
            rows.append(deepcopy(candidate)); by_id[identifier] = rows[-1]; restored += 1
            continue
        for field, value in candidate.items():
            if field in {"auth", "updated_at"}:
                continue
            if _missing(existing.get(field)) and not _missing(value):
...
def recover_assets_if_degraded():
    ...
    preg = checkpoint.get("registry") if isinstance(checkpoint.get("registry"), dict) else {}
    pdev = checkpoint.get("device_pool") if isinstance(checkpoint.get("device_pool"), dict) else {}
    q, pq = _asset_quality(registry, devices), _asset_quality(preg, pdev)
    if q[0] >= pq[0] and q[3] >= pq[3]:
        return {"restored": False, "reason": "current_not_degraded"}
    ra = _merge_missing(registry, preg, "accounts")
    rd = _merge_missing(devices, pdev, "devices")
    if not (ra or rd):
        return {"restored": False, "reason": "nothing_missing_after_merge"}
    registry["schema"], devices["schema"] = SCHEMA, DEVICE_SCHEMA
...
def _account_snapshot(account):
    ...
    return {
        "account_id": account.get("account_id"), "platform": account.get("platform"),
        "platform_name": account.get("platform_name"), "display_name": account.get("display_name"),
        "auth_status": auth.get("status"), "auth_method": auth.get("method"),
        "platform_subject_id": auth.get("platform_subject_id"), "authorized_scopes": list(auth.get("scopes") or []),
        "expires_at": auth.get("expires_at"), "last_verified_at": auth.get("last_verified_at"),
        "reauthorization_required": bool(auth.get("reauthorization_required")),
        "service_scope": account.get("service_scope") or {}, "region_scope": account.get("region_scope") or {},
        "preferred_device_id": account.get("preferred_device_id"), "legacy_account_ids": account.get("legacy_account_ids") or [],
        "source": account.get("source"), "created_at": account.get("created_at"), "updated_at": account.get("updated_at"),
    }


def _legacy_auth(existing, legacy):
    auth = deepcopy((existing or {}).get("auth") or {})
    incoming = _auth_state(legacy.get("login_status"))
    # Stale legacy not_verified must not downgrade a durable connected identity.
    if auth.get("status") != "connected" or incoming == "connected":
        auth["status"] = incoming
    auth.setdefault("method", "legacy_real_device")
    if incoming == "connected":
        auth.update({"method": "real_device_verified", "last_verified_at": legacy.get("login_verified_at") or auth.get("last_verified_at") or now_iso(), "reauthorization_required": False})
    else:
        auth.setdefault("reauthorization_required", auth.get("status") != "connected")
...
def migrate_legacy_assets():
    ...
    registry, devices = _read_registry(), _read_devices()
    changed, device_map, migrated = False, {}, []
    ...
    for legacy in state.get("devices", []):
        ...
        stable = _stable_device_id(legacy_id); device_map[legacy_id] = stable
        existing = next((x for x in devices["devices"] if x.get("device_id") == stable), None)
        row = {
            "device_id": stable, "legacy_device_id": legacy_id, "label": legacy.get("label") or legacy_id,
            "device_type": "real_android", "connection": legacy.get("connection") or "unknown",
            "health": legacy.get("health") or "unknown", "probe_source": legacy.get("probe_source"),
            "last_seen_at": legacy.get("last_seen_at"), "created_at": (existing or {}).get("created_at") or legacy.get("created_at") or now_iso(),
            "updated_at": now_iso(),
        }
        if existing:
            before = deepcopy(existing); existing.update(row); changed = changed or before != existing
        else:
            devices["devices"].append(row); changed = True
    ...
    for legacy in state.get("accounts", []):
        ...
        legacy_id = str(legacy.get("account_id") or "").strip()
        existing = next((x for x in registry["accounts"] if legacy_id and legacy_id in (x.get("legacy_account_ids") or [])), None)
        if existing is None:
            existing = next((x for x in registry["accounts"] if x.get("platform") == platform and x.get("display_name") == alias), None)
        ...
        row = {
            ...
            "display_name": alias, "auth": _legacy_auth(existing, legacy),
            "service_scope": {"all_local_services": bool(all_services), "services": [] if all_services else services[:50]},
            "region_scope": {"all_regions": bool((existing or {}).get("region_scope", {}).get("all_regions")), "regions": regions[:50]},
            "preferred_device_id": (existing or {}).get("preferred_device_id") or device_map.get(str(legacy.get("device_id") or "").strip()),
            "legacy_account_ids": legacy_ids, "source": (existing or {}).get("source") or "r8_legacy_migration",
            "created_at": (existing or {}).get("created_at") or legacy.get("created_at") or now_iso(), "updated_at": now_iso(),
        }
        if existing:
            before = deepcopy(existing); existing.update(row); changed = changed or before != existing
        else:
            registry["accounts"].append(row); changed = True
        migrated.append(account_id)

    registry["schema"], devices["schema"] = SCHEMA, DEVICE_SCHEMA
    registry["updated_at"], devices["updated_at"] = now_iso(), now_iso()
    if changed:
        registry["migrations"].append({"at": now_iso(), "kind": "r8_legacy_account_device_import", "migrated_accounts": sorted(set(migrated)), "truth": "迁移只建立稳定身份映射；不生成平台登录、发布或授权成功。"})
        registry["migrations"] = registry["migrations"][-100:]
    write_json(REGISTRY_PATH, registry); write_json(DEVICE_PATH, devices)
    _save_checkpoint(registry, devices, "legacy_asset_migration")
...
def snapshot(*, skip_migration=False):
    ...
    return {
        ...
        "summary": {
            "accounts": len(accounts), "connected_accounts": sum(1 for x in accounts if (x.get("auth") or {}).get("status") == "connected"),
            ...
            "online_devices": sum(1 for x in drows if x.get("connection") == "connected"),
        },
...
def update_scope(account_id, *, all_services=None, services=None, all_regions=None, regions=None, preferred_device_id=None):
    ...
    account["updated_at"], registry["updated_at"] = now_iso(), now_iso()
    write_json(REGISTRY_PATH, registry); _save_checkpoint(registry, devices, "account_scope_or_device_change")
...
def set_auth_state(account_id, status, *, method=None, platform_subject_id=None, scopes=None, expires_at=None):
    ...
    account = next((x for x in registry.get("accounts", []) if x.get("account_id") == account_id), None)
    if not account: raise ValueError("账号资产不存在")
    auth = account.setdefault("auth", {}); previous = auth.get("status")
    auth["status"], auth["reauthorization_required"] = status, status == "needs_authorization"
    ...
    account["updated_at"], registry["updated_at"] = now_iso(), now_iso()
    write_json(REGISTRY_PATH, registry); _save_checkpoint(registry, devices, "authorization_state_change")
    _append_audit("authorization_state_changed", account_id=account_id, detail={"from": previous, "to": status, "method": auth.get("method")})
    return _account_snapshot(account)


def register_official_identity(platform, display_name, platform_subject_id, *, method="oauth"):
    platform, display_name, subject = str(platform or "").strip(), str(display_name or "").strip(), str(platform_subject_id or "").strip()
    if not platform or not display_name or not subject: raise ValueError("官方账号身份信息不完整")
    registry, devices = _read_registry(), _read_devices()
    existing = next((x for x in registry.get("accounts", []) if x.get("platform") == platform and (x.get("auth") or {}).get("platform_subject_id") == subject), None)
    ...
    if existing is None:
        existing = {
            "account_id": _stable_account_id(platform, subject), "platform": platform,
            "platform_name": r8_control.PLATFORMS.get(platform, platform), "display_name": display_name,
            "auth": {}, "service_scope": {"all_local_services": False, "services": []},
            "region_scope": {"all_regions": False, "regions": []}, "preferred_device_id": None,
            "legacy_account_ids": [], "source": "official_authorization", "created_at": now_iso(),
        }
    ...
    existing["display_name"], existing["updated_at"], registry["updated_at"] = display_name, now_iso(), now_iso()
    write_json(REGISTRY_PATH, registry); _save_checkpoint(registry, devices, "official_identity_registered")
