...
ACTIVE_TASK_STATES = {"queued", "staging", "ready_for_composer", "ready_for_owner_final_publish", "needs_human"}
...
def queue_plan(plan: dict, route_or_account: dict, video: dict) -> dict:
    """Queue one approved plan on its selected real device, idempotently."""
    ...
    if not adb_device_id:
        return {"queued": False, "reason": "account_has_no_execution_device", "device_asset_id": stable_device_id or None, "publishes_content": False}
    ...
    if not device or not device.get("connected"):
        return {"queued": False, "reason": "execution_device_offline", "device_asset_id": stable_device_id or None, "adb_device_id": adb_device_id, "publishes_content": False}
