(() => {
  ...
  const FRAME_SRC = '/r8_13_seo_geo.html?embed=1';
  const MIN_FRAME_HEIGHT = 1300;
  ...
  function ensureUiStyle(doc) {
    if (!doc || doc.getElementById('kz-seo-action-style')) return;
  ...
  function toast(frame, message, kind='info', hold=4200) {
    try {
      ...
      console.warn('SEO/GEO feedback deferred', error);
    }
  }

  function baseModal(frame, html) {
    const doc = frame?.contentDocument;
    if (!doc) throw new Error('SEO/GEO 页面尚未加载完成');
    ensureUiStyle(doc);
  ...
  function injectScript(frame, id, src) {
    try {
      const doc = frame?.contentDocument;
      ...
      console.warn(`SEO/GEO module inject deferred: ${src}`, error);
    }
  }

  function safeResize(frame) {
    try {
      const doc = frame?.contentDocument;
      if (!doc) return;
      const raw = Math.max(
      ...
      const desired = clamp(raw, MIN_FRAME_HEIGHT, MAX_FRAME_HEIGHT);
      const current = parseInt(frame.style.height || '0', 10) || 0;
      if (Math.abs(desired - current) >= 8) frame.style.height = `${desired}px`;
    } catch (error) {
      console.warn('SEO/GEO safe resize deferred', error);
    }
  }

  function scheduleResize(frame) {
    clearTimeout(frame.__kzResizeTimer);
    frame.__kzResizeTimer = setTimeout(() => safeResize(frame), 80);
  }

  function installFiniteFrameResize(frame) {
    if (!frame || frame.__KZ_SAFE_RESIZE_INSTALLED__) return;
    frame.__KZ_SAFE_RESIZE_INSTALLED__ = true;
    const delays = [0, 120, 350, 800, 1600, 3000, 6000];
    delays.forEach(delay => setTimeout(() => scheduleResize(frame), delay));
    window.addEventListener('resize', () => scheduleResize(frame));
  }
  ...
  async function refreshFrame(frame) {
    try {
      ...
      console.warn('SEO/GEO refresh deferred', error);
    }
  }
  ...
  async function ensureGoogleAppCredentials(frame) {
    ...
    return new Promise(resolve => {
      ...
      const modal = baseModal(frame, `
        ...
        <p>只需配置一次 Google OAuth Web 应用。Client Secret 会写入当前 Windows 用户的 DPAPI 加密凭据库。</p>
        ...
        <div class="kz-seo-field"><label>Google Cloud 重定向 URI</label><input value="${callback.replace(/&/g,'&amp;').replace(/"/g,'&quot;')}" readonly></div>
        <div class="kz-seo-hint">Google Cloud：启用 Search Console API → 创建 OAuth 2.0 Client（Web application）→ Authorized redirect URIs 添加上面整行。</div>
      ...
      modal.querySelector('#kz-google-save').onclick = async () => {
        const button = modal.querySelector('#kz-google-save');
        ...
        if (!clientId || !clientSecret) {
          msg.textContent = 'Client ID 和 Client Secret 都必须填写。';
          msg.className = 'kz-seo-modal-msg error';
          return;
        }
        ...
        try {
          ...
          msg.textContent = `保存失败：${error.message || String(error)}`;
          msg.className = 'kz-seo-modal-msg error';
  ...
  async function configureBaidu(frame) {
    return new Promise(resolve => {
      const modal = baseModal(frame, `
        <h2>配置百度搜索资源平台</h2>
        <p>请复制百度“资源提交 → 普通收录 → API提交”的完整接口调用地址。准入密钥只保存到本机 Windows DPAPI。</p>
        ...
        <div class="kz-seo-hint">不要把准入密钥发到聊天、GitHub 或截图中。</div>
      ...
      modal.querySelector('#kz-baidu-save').onclick = async () => {
        ...
        try { parsed = new URL(raw); } catch (error) {
          msg.textContent = '接口地址格式不正确，请从百度后台完整复制。';
          msg.className = 'kz-seo-modal-msg error';
          return;
        }
        const site = (parsed.searchParams.get('site') || '').trim();
        const token = (parsed.searchParams.get('token') || '').trim();
        if (parsed.hostname !== 'data.zz.baidu.com' || !site || !token) {
          msg.textContent = '没有识别到百度 API 的 site/token，请重新复制完整接口调用地址。';
          msg.className = 'kz-seo-modal-msg error';
          return;
        }
        ...
        try {
          ...
          modal.remove();
          toast(frame, count > 0 ? `百度 API 已连接，并取得 ${count} 个真实提交回执。` : `百度配置已保存；本轮提交 ${count} 个，失败 ${failed} 个。`, count > 0 ? 'ok' : 'info', 8000);
          resolve(true);
        } catch (error) {
          msg.textContent = `百度配置/提交失败：${error.message || String(error)}`;
          msg.className = 'kz-seo-modal-msg error';
          setBusy(button, false);
        }
      };
  ...
  async function openSearchAuth(frame, platform) {
    if (platform === 'baidu_search_resource') return configureBaidu(frame);
  ...
  function installControlReliability(frame) {
    try {
      ...
      doc.addEventListener('click', async event => {
        ...
        if (auth) {
          ...
          try {
            ...
            if (opened && platform !== 'baidu_search_resource') toast(frame, '授权入口已打开，请在官方页面完成授权。', 'ok', 5200);
          } catch (error) {
      ...
      console.warn('SEO/GEO control reliability install deferred', error);
    }
  }
  ...
  function configureFrame(frame) {
    ...
    const onLoad = () => {
      try {
        injectScript(frame, 'r814-autonomy-script', '/r8_14_seo_geo_autonomy_ui.js');
        injectScript(frame, 'r812-search-auth-script', '/r8_12_multi_account_auth_ui.js');
        installControlReliability(frame);
        installFiniteFrameResize(frame);
        scheduleResize(frame);
      } catch (error) {
        console.warn('SEO/GEO iframe setup deferred', error);
      }
    };
  ...
  function ensurePage(createFrame=false) {
    let page = document.getElementById(PAGE_ID);
    if (!page) {
      page = document.createElement('section');
      page.id = PAGE_ID;
      page.className = 'page';
      page.dataset.kzLazySeo = '1';
      document.querySelector('main')?.appendChild(page);
    }
    if (createFrame && !page.querySelector(`#${FRAME_ID}`)) {
      const frame = document.createElement('iframe');
      frame.id = FRAME_ID;
      frame.title = 'SEO/GEO增长中心';
      frame.src = FRAME_SRC;
      frame.style.cssText = `width:100%;height:${MIN_FRAME_HEIGHT}px;border:0;background:#f4f7fb;pointer-events:auto`;
      frame.scrolling = 'no';
  ...
  window.addEventListener('message', event => {
    ...
    const frame = document.getElementById(FRAME_ID);
    const target = frame?.contentDocument?.getElementById(String(event.data.target || ''));
  ...
  // Deliberately do not create the SEO iframe during application startup.
  // This keeps the main navigation responsive even when SEO content is large.
  ensureNav();
  window.addEventListener('kz:app-ready', ensureNav);
  window.addEventListener('r810:workbench-ready', ensureNav);
})();
