...
def record_candidate(payload):
    candidate = _ORIGINAL["record_candidate"](payload)
    if not _is_worker_final(payload):
        return candidate
    qc = candidate.get("technical_qc") or {}
    if not candidate.get("exists") or not qc.get("passed"):
        return candidate
    # Do not expose the candidate to ChatGPT/owner review while local subtitle,
    # voice and second-pass QC are still running.
    cf.update_runtime_state(
        str(payload.get("video_id") or ""),
        status="后期增强中",
        bottleneck="正在完成字幕、可选本地配音与第二次技术质检",
        auto_action="本地执行器完成后自动进入ChatGPT成片质检，无需人工操作",
    )
    data = cf._load()
    video = cf._by_id(data.get("videos", []), str(payload.get("video_id") or ""), "视频任务")
